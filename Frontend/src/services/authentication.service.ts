...
export class AuthenticationService {
        ...
        if (rejected.response.status >= 400 && rejected.response.status < 500) {
          ...
          matSnackbar.open(
            rejected.response.status +
              ...
              'Please check your network connectivity and verify your account credentials',
            'Dismiss',
            { duration: 5000 }
          );
  ...
  async login(req: LoginRequest) {
    const loginAxiosInstance = axios.create({
      baseURL: environment.apiBaseUrl,
    }); // Create a new instance to avoid have meaningful interceptors
      ...
      (rejected) => {
        if (rejected.response.status >= 400 && rejected.response.status < 500) {
          ...
          this.matSnackbar.open(
            rejected.response.status + ': No such user could be found',
            'Dismiss',
            { duration: 5000 }
          );
          throw new Error(
  ...
  async register(req: RegisterRequest): Promise<TokenResponse> {
    const registerAxiosInstance = axios.create({
      baseURL: environment.apiBaseUrl,
    }); // Create a new instance to avoid have meaningful interceptors
    registerAxiosInstance.interceptors.response.use(
      ...
      (rejected) => {
        if (rejected.response.status >= 400 && rejected.response.status < 500) {
          //Client error
          this.matSnackbar.open(
            rejected.response.status + ': Username already taken',
            'Dismiss',
            { duration: 5000 }
          );
        } else if (rejected.response.status > 499) {
    ...
    return await registerAxiosInstance
      .post('auth/register', req, this.headerConfig)
      .then((response) => {
